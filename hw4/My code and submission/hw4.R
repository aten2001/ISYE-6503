...
#There are correlations between the predictors, so PCA is a good choose.
...
#The R-square of this model is 0.106, the crime rate for the city with given data is 1112.678 
...
#I would say although PCA method addressed for the correlations between the predcitors and ranked the coordinates by importance. It didn't 
#address for the overfitting, which is a big issue in our data. 
...
#This model doesn't show a significant improvement compared to the previous model.
#but since it is not a real model, it is the average of all the different trees, which is better than just
#one tree.
...
setwd("G:/a-XiaoWang/OMSA/ISYE-6501/hw4")
